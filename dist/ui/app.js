...
import { PlayMode, LoopMode, DEFAULT_STATE, DEFAULT_EQ_BANDS, EQ_PRESETS, formatTime, clamp, ModMode, DEFAULT_SETTINGS, } from '../types.js';
...
export class App {
    ...
    async init() {
        ...
        if (this.state.currentPlaylistId) {
            ...
            if (allPl.length > 0)
                this.plMgr.setCurrentPlaylist(allPl[0].id);
        }
        this.renderPlaylist();
    ...
    initLevelMeter() {
        ...
        const startMeter = () => { if (!this.levelRaf && !document.hidden)
            this.levelRaf = requestAnimationFrame(tick); };
        document.addEventListener('visibilitychange', () => {
            if (document.hidden) {
                if (this.levelRaf) {
    ...
    createControls() {
        ...
        this.waveform.onClick = (t) => { if (this.engine.duration > 0)
            this.engine.seek(t * this.engine.duration); };
        this.waveform.onLoopSelect = (s, e) => {
            this.state.loopStart = s;
            this.state.loopEnd = e;
            this.state.loopMode = LoopMode.AB;
            this.engine.setLoop(s, e);
        ...
        const sel = this.$['eq-preset-select'];
        if (sel) {
            ...
            sel.addEventListener('change', () => {
                ...
                if (preset) {
                    this.state.eqGains = [...preset.bands];
    ...
    wireButtons() {
        ...
        const modSel = this.$['mod-mode-select'];
        modSel?.addEventListener('change', () => {
            const map = { fl: ModMode.Fl, ch: ModMode.Ch, ph: ModMode.Ph };
        ...
        window.addEventListener('keydown', (e) => {
            ...
            switch (e.code) {
                case 'Space':
                    ...
                    break;
                case 'ArrowLeft':
                    this.engine.seek(Math.max(0, this.currentTime() - 5));
                    break;
                case 'ArrowRight':
                    this.engine.seek(this.currentTime() + 5);
                    break;
                case 'ArrowUp':
                    e.preventDefault();
                    this.adjustTempo(5);
    ...
    async loadTrack(id, autoPlay) {
        ...
        try {
            ...
            if (!meta || !audioData || this._loadingId !== id)
                return;
            this.currentTrack = meta;
            this.currentAudioData = audioData;
            this.state.currentTrackId = id;
            this.plMgr.setCurrentTrack(id);
            ...
            if (this.settings.bpmAuto && this.currentAudioData)
                this.engine.detectBPM(this.currentAudioData.slice(0));
        }
        catch (err) {
    ...
    doStop() {
        ...
        this.viz.setPlaying(false);
        this.updatePlayButton();
        this.$['time-current'].textContent = '0:00.0';
        this.waveform.setPosition(0);
    }
    async doNext() { const id = this.plMgr.getNextTrackId(); if (id)
        await this.loadTrack(id, true); }
    async doPrev() { const id = this.plMgr.getPrevTrackId(); if (id)
        await this.loadTrack(id, true); }
    _lastPos = 0;
    currentTime() { return this._lastPos; }
    _posRafPending = false;
    _lastPosCurrent = 0;
    _lastPosTotal = 0;
    _pitchTickCount = 0;
    onEngineEvent(ev) {
        switch (ev.type) {
            case 'position': {
                this._lastPosCurrent = ev.current;
                this._lastPosTotal = ev.total;
                if (!this._posRafPending) {
                    ...
                    requestAnimationFrame(() => {
                        ...
                        if (this._lastPosTotal > 0)
                            this.waveform.setPosition(this._lastPosCurrent / this._lastPosTotal);
                        this._pitchTickCount = (this._pitchTickCount + 1) % 12;
                        if (this._pitchTickCount === 0) {
                            const pd = this.$['pitch-display'];
                            if (pd) {
                                const p = this.engine.detectPitch();
                                pd.textContent = p ? `${p.note} ${p.hz}Hz` : '';
                            }
                        }
                    });
                }
            ...
            case 'ended':
                ...
                break;
            case 'loaded':
                this.$['time-total'].textContent = formatTime(ev.duration);
                break;
            case 'bpm': {
                this.state.bpm = ev.value;
                const bE = this.$['bpm-display'];
                if (bE)
                    bE.textContent = ev.value > 0 ? `${ev.value} BPM` : '-- BPM';
                this.saveState();
                break;
            }
        }
    ...
    async handleTrackEnd() {
        switch (this.state.loopMode) {
            case LoopMode.Single:
                ...
                return;
            case LoopMode.AB:
                this.engine.seek(this.state.loopStart);
                this.engine.play();
                return;
        }
        const nextId = this.plMgr.getNextTrackId();
        if (nextId) {
            await this.loadTrack(nextId, true);
        }
        else if (this.state.playMode === PlayMode.RepeatAll) {
            const pl = this.plMgr.getCurrent();
            if (pl && pl.trackIds.length > 0) {
                this.plMgr.setCurrentIndex(0);
                await this.loadTrack(pl.trackIds[0], true);
            }
        }
        else {
            this.playing = false;
    ...
    cycleLoopMode() {
        ...
        if (this.state.loopMode === LoopMode.Single && this.currentTrack)
            this.engine.setLoop(0, this.currentTrack.duration);
        else if (this.state.loopMode === LoopMode.AB && this.state.loopStart > 0) {
            this.engine.setLoop(this.state.loopStart, this.state.loopEnd);
            this.waveform.setLoop(this.state.loopStart, this.state.loopEnd);
        }
        else if (this.state.loopMode === LoopMode.None) {
            this.engine.clearLoop();
            this.waveform.clearLoop();
        }
        this.updateLoopButton();
    ...
    adjustTempo(delta) {
        ...
        this.state.tempo = newVal / 100;
        this.engine.setTempo(this.state.tempo);
        this.tempoKnob.value = newVal;
        this.saveState();
    }
    updatePlayButton() {
        const btn = this.$['btn-play'];
        if (btn) {
            btn.innerHTML = ico(this.playing ? 'pause' : 'play');
    ...
    renderPlaylist() {
        ...
        const renderRange = () => {
            ...
            for (let idx = startIdx; idx < endIdx; idx++) {
                ...
                let li = scroller.querySelector(`[data-tid="${tid}"]`);
                if (!li) {
                ...
                const eqEl = li.querySelector('.track-item-eq');
                if (eqEl)
                    eqEl.style.display = isPlaying ? '' : 'none';
                li.querySelector('.track-item-idx').textContent = String(idx + 1);
            }
            Array.from(scroller.querySelectorAll('.track-item')).forEach(el => {
                if (!rendered.has(el.dataset.tid))
                    el.remove();
            });
        };
        renderRange();
    ...
    highlightCurrentTrack() {
        ...
        for (const li of items) {
            if (li.dataset.tid === this.currentTrack.id) {
                ...
                const eq = li.querySelector('.track-item-eq');
                if (eq)
                    eq.style.display = '';
                break;
            }
    ...
    async exportWAV() {
        if (!this.currentAudioData || !this.currentTrack) {
        ...
        try {
            const rendered = await this.engine.renderOffline(this.currentAudioData, this.state.pitch, this.state.tempo, this.state.eqGains, this.state.vocalCancel, { bits: this.state.bitcrusherBits, reduction: this.state.bitcrusherReduction, enabled: this.state.bitcrusherEnabled }, this.state.reverb);
    ...
    saveState() {
        try {
            localStorage.setItem('wf-state', JSON.stringify(this.state));
        }
        catch { }
    }
    loadState() {
        try {
            ...
            if (raw) {
                ...
                if (this.state.bpm > 0) {
                    const bE = this.$['bpm-display'];
                    if (bE)
                        bE.textContent = `${this.state.bpm} BPM`;
                }
            }
        }
        catch { }
        try {
            const raw = localStorage.getItem('wf-settings');
            if (raw)
    ...
    saveSettings() {
        try {
            localStorage.setItem('wf-settings', JSON.stringify(this.settings));
        }
        catch { }
    }
    openSettings() {
      ...
      <button id="set-close" style="margin-top:16px;width:100%;padding:10px;background:#f06020;border:none;color:#fff;border-radius:8px;font-size:.85rem;cursor:pointer;font-weight:600;min-height:44px">閉じる</button>`;
        ...
        p.querySelector('#set-formant').addEventListener('change', (e) => { this.settings.formantDef = e.target.checked; this.saveSettings(); });
        p.querySelector('#set-bpm-auto').addEventListener('change', (e) => { this.settings.bpmAuto = e.target.checked; this.saveSettings(); });
        p.querySelector('#set-viz').addEventListener('change', (e) => { this.settings.vizMode = e.target.value; this.saveSettings(); });
        p.querySelector('#set-hue').addEventListener('input', (e) => {
            const v = parseInt(e.target.value);
            this.settings.themeHue = v;
            p.querySelector('#hue-val').textContent = String(v);
        ...
        if ('storage' in navigator && 'estimate' in navigator.storage) {
            navigator.storage.estimate().then(({ usage = 0, quota = 1 }) => {
                ...
                const bar = p.querySelector('#storage-bar');
                const lbl = p.querySelector('#storage-label');
                if (bar) {
                    bar.style.width = `${pct}%`;
                    bar.style.background = pct > 80 ? '#e53935' : '#f06020';
                }
                if (lbl)
                    lbl.textContent = `${usedMB} MB 使用中 / 約 ${totalMB} MB (${pct.toFixed(1)}%)`;
            });
        }
        else {
            const lbl = p.querySelector('#storage-label');
            if (lbl)
                lbl.textContent = 'このブラウザは非対応です';
        }
    }
}
