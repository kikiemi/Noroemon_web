...
export class RotaryKnob {
  ...
  private valueLabel!: HTMLInputElement;
  ...
  private _startAngle = -135;
  private _endAngle = 135;
  ...
  constructor(
    ...
    disc.setAttribute('cx', '50'); disc.setAttribute('cy', '50'); disc.setAttribute('r', '28');
  ...
  private attachEvents(unit: string): void {
    ...
    this.valueLabel.addEventListener('keydown', (e: KeyboardEvent) => {
      if (e.key === 'Enter') { this.valueLabel.blur(); }
      if (e.key === 'Escape') {
        this.valueLabel.setAttribute('readonly', '');
        this.valueLabel.value = this.formatValue(this._value, unit);
...
export class WaveformDisplay {
  ...
  private negPeaks: Float32Array = new Float32Array(0);
  private _position = 0;
  private _duration = 0;
  private _loopStart = -1;
  private _loopEnd = -1;
  private _abSelecting = false;
  private _abStartX = 0;
  private _peakWorker: Worker | null = null;

  onClick: ((position: number) => void) | null = null;
  onLoopSelect: ((start: number, end: number) => void) | null = null;
  ...
  setAudioBuffer(buffer: AudioBuffer): void {
    ...
    if (this._peakWorker) { this._peakWorker.terminate(); this._peakWorker = null; }
    ...
    w.onmessage = (e) => {
      ...
      if (this._peakWorker === w) this._peakWorker = null;
    };
  }
...
export class EQSliderBank {
  private container: HTMLElement;
  private sliders: { el: HTMLElement; fill: HTMLElement; value: number; }[] = [];
  onChange: ((index: number, value: number) => void) | null = null;

  constructor(parent: HTMLElement, labels: readonly number[], initialGains: number[]) {
    this.container = document.createElement('div');
